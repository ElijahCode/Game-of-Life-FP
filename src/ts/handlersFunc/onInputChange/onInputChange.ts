...
export function onInputChange (cb: (width: number, height: number, state: number[][]) => number[][]) {
    const tableElem: HTMLTableElement = document.querySelector('.gameField');
    const gameControllersBlock: HTMLDivElement = document.querySelector('.gameControllers'); 
